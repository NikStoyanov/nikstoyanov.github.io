class Strava::Models::Map < Strava::Model
    ...
    def image_url
      ...
      "https://maps.googleapis.com/maps/api/staticmap?maptype=terrain&path=enc:#{summary_polyline}&key=#{google_maps_api_key}&size=800x800&scale=2&markers=color:yellow|label:S|#{start_latlng[0]},#{start_latlng[1]}&markers=color:green|label:F|#{end_latlng[0]},#{end_latlng[1]}"
    end
  end